...
def F2(x):
    ...
    lambda1 = float(x[len(x)-1]*0.5)+0.5
    alpha = float(x[len(x)-2]*0.00009)+0.0001
    bylevel = float(x[len(x)-3]*.0001)+0.999
    bytree = float(x[len(x)-4]*.0001)+0.999
    subsample1 = float(x[len(x)-5]*.01)+0.99
    minChWeight = int(x[len(x)-6]*5)+1
    gamma1 = float(x[len(x)-7]*0.00009)+0.0001
    NoEstimator = int(x[len(x)-8]*50)+100
    learnRate = float(x[len(x)-9]*0.05)+0.03
    maxDepth = int(x[len(x)-10]*12)+3
    ...
    train2 = x_train.iloc[:, lambda train: q]
    f = q[q!=0]
    ...
    reg_alpha=alpha,reg_lambda=lambda1,silent=True, verbose=False)
    kfold = KFold(n_splits=5)
    results = cross_val_score(xgbb,train2, y_train, cv=kfold,scoring='neg_mean_squared_error')
    rmse=math.sqrt(-results.mean())
